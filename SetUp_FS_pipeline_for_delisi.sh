...
NEW_DIR=/rfanfs/pnl-a/pnl/Collaborator/Delisi
...
echo -n ${NEW_DIR}/dicom_to_nifti/\$case/data/T1/\$case-T1.nrrd > config/T1S_FILEPATTERN
...
mkdir scripts
cp -r ${ORG_PPL_DIR}/realign-edrmasks-pipeline/scripts/applyxfm.sh .
#[config/XFM]
...
#[config/REALIGN_T1]
echo -n ${NEW_DIR}/dicom_to_nifti/\$case/data/T1/\$case-T1.nrrd > config/REALIGN_T1
...
echo -n ${NEW_DIR}/dicom_to_nifti/\$case/data/T1/\$case-T1.nrrd > config/REALIGN_T1
#[config/T2]
echo -n ${NEW_DIR}/dicom_to_nifti/\$case/data/T2/\$case-T2.nrrd > config/T2
